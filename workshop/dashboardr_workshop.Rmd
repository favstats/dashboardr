...
author: "A Comprehensive Hands-On Workshop"
date: "`r Sys.Date()`"
output:
...
|----------|---------------|------------|-------------|
| **Shiny** | ✅ Yes, 24/7 | Slower | Unlimited |
| **dashboardr** | ❌ No | Instant | Pre-defined |
...
- Same bricks → different structures
...
.diagram[
┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│  STEP 1: BUILD  │  -->  │ STEP 2: ASSEMBLE│  -->  │ STEP 3: GENERATE│
│                 │       │                 │       │                 │
│  create_viz()   │       │ create_dashboard│       │generate_dashboard
│  add_viz()      │       │ add_page()      │       │                 │
└─────────────────┘       └─────────────────┘       └─────────────────┘
]
...
|------|----------|----------|
| `histogram` | `create_histogram()` | Distributions |
| `bar` | `create_bar()` | Categories |
| `stackedbar` | `create_stackedbar()` | Composition |
| `stackedbars` | `create_stackedbars()` | Likert questions |
| `timeline` | `create_timeline()` | Time trends |
| `heatmap` | `create_heatmap()` | 2D patterns |
| `treemap` | `create_treemap()` | Hierarchies |
| `scatter` | `create_scatter()` | Relationships |
...
| `timeline` | `time_var`, `response_var` | `group_var` |
...
.tip[
Same parameters work in both `create_histogram()` and `add_viz(type = "histogram")`!
]
...
* response_filter_label = "High Satisfaction (4-5)"
)
```
...
create_heatmap(
  ...
  value_var = "satisfaction",
  agg_fun = mean,
  title = "Avg Satisfaction: Department × Region"
)
```
...
trends <- create_viz(type = "timeline") %>%
  add_viz(time_var = "year", response_var = "score", tabgroup = "trends")
...
x_var           # X variable
y_var           # Y variable (scatter, heatmap)
