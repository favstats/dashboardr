...
all_viz <- bar_viz %>%
  ...
  add_pagination() %>%
  # Removed duplicate combine_viz(timeline_viz) - was causing duplicate modal IDs
...
dashboard <- create_dashboard(
  ...
  mobile_toc = T, plausible = "pa-UnPiJwxFi8TS-XAvCdgQx",
...
# setwd("iconify_tabs_demo")

## Then run the publish workflow (only once for initial setup):
if (FALSE) {
  publish_dashboard(
...
## For subsequent updates after regenerating the dashboard:
if (FALSE) {
  update_dashboard(message = "Update iconify demos")
}
