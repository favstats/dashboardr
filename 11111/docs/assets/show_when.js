...
(function() {
  ...
  function fixChartMinHeight() {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/cbfd47d0-c39e-4a3e-892f-ab3041f60f5c',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'show_when.js:fixChartMinHeight',message:'enter',data:{readyState:document.readyState},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'H3-H4'})}).catch(function(){});
    // #endregion
    try {
      var minH = getComputedStyle(document.documentElement)
                  .getPropertyValue('--chart-min-height').trim() || '400px';
      var grids = document.querySelectorAll('.sidebar-content .bslib-grid[style*="grid-template-rows"]');
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/cbfd47d0-c39e-4a3e-892f-ab3041f60f5c',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'show_when.js:fixChartMinHeight',message:'grids-found',data:{minH:minH,gridCount:grids.length},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'H3-H5'})}).catch(function(){});
      // #endregion
      grids.forEach(function(grid) {
        var before = grid.style.gridTemplateRows;
        ...
        var after = getComputedStyle(grid).gridTemplateRows;
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/cbfd47d0-c39e-4a3e-892f-ab3041f60f5c',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'show_when.js:fixChartMinHeight',message:'grid-override',data:{before:before,after:after,minH:minH},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'H3-H5'})}).catch(function(){});
        // #endregion
      });
    } catch(e) {
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/cbfd47d0-c39e-4a3e-892f-ab3041f60f5c',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'show_when.js:fixChartMinHeight',message:'ERROR',data:{error:e.message},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'H3'})}).catch(function(){});
      // #endregion
    }
  }
