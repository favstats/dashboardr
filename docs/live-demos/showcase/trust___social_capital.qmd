---
title: "{{< iconify ph handshake >}} Trust & Social Capital"
format: dashboard
---

```{r, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
dashboardr::.page_config(accessibility = TRUE, inputs = TRUE, sidebar = TRUE)
```

Use the filters to explore regional patterns in social trust.

```{r setup}
#| echo: false
#| warning: false
#| message: false
#| error: false
#| results: 'hide'

# Load dashboardr (includes dplyr, highcharter as dependencies)
library(dashboardr)

# Global chunk options
knitr::opts_chunk$set(
  echo = FALSE,
  warning = FALSE,
  message = FALSE,
  error = FALSE,
  fig.width = 12,
  fig.height = 8,
  dpi = 300
)

# Load data
data <- readRDS('dataset_2849obs.rds')

# Data summary
cat('Dataset loaded:', nrow(data), 'rows,', ncol(data), 'columns\n')

```


## {.sidebar width="280px"}

### Filter Data


```{r}
#| echo: false
#| results: 'asis'
dashboardr::render_input(
  input_id = "education_filter",
  label = "Education Level:",
  type = "radio",
  filter_var = "degree",
  options = c("All", "less than high school", "high school", "associate/junior college", "bachelor's", "graduate"),
  options_from = NULL,
  default_selected = c("less than high school", "high school", "associate/junior college", "bachelor's", "graduate"),
  placeholder = "Select...",
  width = "300px",
  min = 0,
  max = 100,
  step = 1,
  value = NULL,
  show_value = TRUE,
  inline = TRUE,
  stacked = TRUE,
  stacked_align = "center",
  group_align = "center",
  ncol = 2,
  nrow = NULL,
  toggle_series = NULL,
  override = FALSE,
  labels = NULL,
  size = "md",
  help = NULL,
  disabled = FALSE
)
```


```{r}
#| echo: false
#| results: 'asis'
dashboardr::render_input(
  input_id = "gender_filter",
  label = "Gender:",
  type = "checkbox",
  filter_var = "sex",
  options = c("male", "female"),
  options_from = NULL,
  default_selected = c("male", "female"),
  placeholder = "Select...",
  width = "300px",
  min = 0,
  max = 100,
  step = 1,
  value = NULL,
  show_value = TRUE,
  inline = TRUE,
  stacked = TRUE,
  stacked_align = "center",
  group_align = "center",
  ncol = 2,
  nrow = NULL,
  toggle_series = NULL,
  override = FALSE,
  labels = NULL,
  size = "md",
  help = NULL,
  disabled = FALSE
)
```


## Column


### Row

#### Happiness by Region

```{r stackedbar-region-happy}
# Happiness by Region
result <- viz_stackedbar(
  data = data,
  title = "Happiness by Region",
  x_var = "region",
  stack_var = "happy",
  subtitle = "Filter by education and gender to see how happiness varies",
  x_label = "Region",
  y_label = "Percentage",
  stack_label = "Happiness",
  stacked_type = "percent",
  stack_order = c("very happy", "pretty happy", "not too happy"),
  color_palette = c("#27ae60", "#f39c12", "#e74c3c"),
  cross_tab_filter_vars = c("degree", "sex")
)

# Set chart height
if (inherits(result, 'highchart')) {
  result <- highcharter::hc_size(result, height = 550)
}

result <- dashboardr:::.embed_cross_tab(result)

# Force container height with explicit wrapper
result <- htmltools::div(
  style = 'height: 550px !important; min-height: 550px !important; width: 100%; overflow: visible;',
  result
)

result
```

