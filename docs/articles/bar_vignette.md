...
`bar_type = "mean"` with error bars to show statistical uncertainty.
...
For more control, use format strings or the
...
| `tooltip` | Full tooltip config (see [`?tooltip`](https://favstats.github.io/dashboardr/reference/tooltip.md)) | `tooltip(format = "...")` |
...
for the complete tooltip customization guide.
...
grouped/clustered bars - Showing means with error bars (SD, SE, or CI)
