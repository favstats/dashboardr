...
This vignette covers the customization options available across
...
Use `color_palette` to control the colors used in any visualization. You
can pass:
...
helper gives you full control over hover information.
...
Toggle data labels with `data_labels_enabled` and control decimal
precision with `label_decimals`:
...
Add error bars to bar charts with `error_bars`. Options include `"se"`
(standard error), `"sd"` (standard deviation), and `"ci"` (confidence
interval):
...
Use theme functions to apply consistent styling across a dashboard.
These are applied at the dashboard level:
...
dashboardr supports multiple rendering backends. Pass `backend` to any
viz function:
...
See
[`vignette("chart-backends")`](https://favstats.github.io/dashboardr/articles/chart-backends.md)
for a full comparison of available backends and their capabilities.
