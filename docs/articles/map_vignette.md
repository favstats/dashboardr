...
colored by data values. They’re perfect for showing how metrics vary
...
**Important**: Maps require data with geographic identifiers (ISO
country codes, state abbreviations, etc.). Unlike other visualizations,
maps typically can’t use GSS data directly since GSS regions don’t map
...
Control the color scale with `color_palette` - provide low and high
colors:
...
**Use bar charts instead when:** - Precise comparisons matter more than
