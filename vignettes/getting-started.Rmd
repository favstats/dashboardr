...
2. **Visualizations** - How you show it
   ...
   create_viz(type = "histogram", color_palette = c("#3498DB")) %>%
     add_viz(x_var = "age", title = "Age", tabgroup = "demographics") %>%
     add_viz(x_var = "income", bins = 50, tabgroup = "demographics")  # Override bins
   ```
...
3. **Layout** - How you organize content
   ...
   viz1 %>% combine_viz(viz2) %>% add_pagination() %>% combine_viz(viz3)
   
   # Rich content with cards and blockquotes
   card("Team Member", content = "Bio...", image = "photo.jpg")
   ```

4. **Navigation** - How users move around
   ```r
...
6. **Content** - Rich text and interactivity
   ```r
   md_text(
     "# Dashboard Overview",
     "**Key findings** with *emphasis*",
     "- {{< iconify ph:check-circle-fill >}} Bullet with icon",
     "```{r}",
     "# Embedded R code that executes!",
     "create_blockquote('Important note', preset = 'info')",
     "```"
   )
   ```
...
my_viz <- create_viz(type = "histogram", color_palette = c("#3498DB")) %>%
