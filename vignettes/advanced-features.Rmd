...
Interactive input widgets transform your dashboard from a static report into an **exploratory data tool**. Users can filter data, adjust parameters, and see visualizations update in real-time - all without writing code.

> **🔗 See it in action:** Check out the [Interactive Inputs Demo](../live-demos/inputs/index.html) to try all input types with real GSS data!

### How It Works

The input system follows a simple pattern:

1. **Create an input row** with `add_input_row()` - this is the container
2. **Add inputs** with `add_input()` - specify what controls users see
3. **Close the row** with `end_input_row()`
4. **Add visualizations** - they automatically respond to the filters

Each input is linked to a variable in your data via `filter_var`. When users interact with the input, the data is filtered and all visualizations on the page update.
...
> **Note:** Inputs appear as placeholders in `preview()`. They become fully interactive when you run `generate_dashboard(render = TRUE)` and view in a browser.

### Input Types

dashboardr supports four input types, each suited for different data types:
...
Dropdown menus allow users to select one or more values to filter the data. Use `select` for single choice, `select_multiple` for multiple:
...
Sliders work for numeric variables, letting users select a value or range:
...
Copy this code to create and view an interactive inputs dashboard. This example combines multiple input types to create a comprehensive data exploration tool:
...
After running this code, a browser window will open with your interactive dashboard. Try selecting different education levels, checking/unchecking races, and toggling genders to see how the charts update!

This will open a browser window where you can interact with the dropdowns and checkboxes to filter the chart in real-time.
...
> **See it in action:** Check out the <a href="../live-demos/overlay/glass_theme.html">Loading Overlay Demo</a> to experience different overlay themes.
