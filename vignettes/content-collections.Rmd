...
<td style="padding: 12px; vertical-align: top;">
Bar, Stacked Bar, Histogram, Timeline, Heatmap, Scatter, Treemap, Map
</td>
<td style="padding: 12px; vertical-align: top;">
...
When you call `create_content()`, any parameters you provide become **defaults** for subsequent `add_viz()` calls:
...
dashboardr supports 9 visualization types. Each is optimized for different data patterns.
...
When you have multiple survey questions with the same response scale (like Likert items), `type = "stackedbars"` displays them together for comparison. This is perfect for "confidence in institutions" batteries or agreement scales.
...
Use `preview()` to quickly check your visualizations and content blocks during development:
