...
.generate_typed_viz <- function(spec) {
  ...
  # Only add cross-tab code when there are interactive inputs that need client-side filtering
  has_cross_tab <- !is.null(spec$cross_tab_filter_vars) && length(spec$cross_tab_filter_vars) > 0
  
  if (has_cross_tab) {
    # Extract cross-tab attributes BEFORE any wrapping that might strip them
    cross_tab_lines <- c(
      "",
