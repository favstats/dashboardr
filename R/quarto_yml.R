...
.generate_quarto_yml <- function(proj) {
  ...
  # 2. Manually switches tabs WITHOUT using Bootstrap API (bypasses internal scroll behavior)
  # 3. Uses stopImmediatePropagation to prevent other handlers from interfering
  # 4. Uses history.replaceState to prevent hash changes
  # 5. Multiple restoration attempts (immediate + RAF + 6 timed attempts)
  # 6. Intercepts scroll events during transition to force position
  # 7. Intercepts shown.bs.tab events at capture phase
  tab_jump_fix_script <- c(
    "        <script>",
    "        (function() {",
    "          'use strict';",
    "          ",
    "          // Scroll lock state - shared across handlers",
    "          var scrollLock = {",
    "            active: false,",
    "            x: 0,",
    "            y: 0",
    "          };",
    "          ",
    "          // Early scroll capture - populated on mousedown before click",
    "          var earlyScrollCapture = { x: 0, y: 0, captured: false };",
    "          ",
    "          // Lock scroll position - freeze the page during tab transition",
    "          function lockScroll() {",
    "            // Use early-captured scroll position if available (more reliable)",
    "            if (typeof earlyScrollCapture !== 'undefined' && earlyScrollCapture.captured) {",
    "              scrollLock.x = earlyScrollCapture.x;",
    "              scrollLock.y = earlyScrollCapture.y;",
    "              earlyScrollCapture.captured = false; // Reset for next click",
    "            } else {",
    "              scrollLock.x = window.scrollX || window.pageXOffset || 0;",
    "              scrollLock.y = window.scrollY || window.pageYOffset || 0;",
    "            }",
    "            scrollLock.active = true;",
    "            ",
    "            // CRITICAL: Lock the body height to prevent scroll clamping",
    "            // When tab content changes height, browser clamps scroll position",
    "            // This prevents that by maintaining minimum height",
    ...
    "            );",
    "            scrollLock.originalMinHeight = document.body.style.minHeight;",
    "            document.body.style.minHeight = docHeight + 'px';",
    "            ",
    "            // Add class to enable CSS-based scroll behavior override",
    "            document.documentElement.classList.add('dashboardr-tab-switching');",
    "            console.log('[dashboardr] Scroll locked at:', scrollLock.y, '(early captured) bodyHeight:', docHeight);",
    "          }",
    "          ",
    "          // Unlock scroll after transition complete",
    "          function unlockScroll() {",
    "            scrollLock.active = false;",
    "            document.documentElement.classList.remove('dashboardr-tab-switching');",
    "            ",
    "            // Restore original body min-height",
    "            document.body.style.minHeight = scrollLock.originalMinHeight || '';",
    "            console.log('[dashboardr] Scroll unlocked');",
    "          }",
    "          ",
    ...
    "            console.log('[dashboardr] Restoring scroll to:', y, 'current:', window.scrollY);",
    "            ",
    "            // Immediate restoration",
    "            window.scrollTo(x, y);",
    "            ",
    "            // Multiple restoration attempts using requestAnimationFrame",
    ...
    "            });",
    "            ",
    "            // Timed restorations for Bootstrap/Quarto async operations",
    "            [0, 16, 32, 50, 100, 150, 200, 250].forEach(function(delay) {",
    "              setTimeout(function() {",
    "                window.scrollTo(x, y);",
    "              }, delay);",
    "            });",
    "            ",
    "            // Final restoration and unlock after all async operations",
    "            setTimeout(function() {",
    "              window.scrollTo(x, y);",
    "              console.log('[dashboardr] Final scroll restore to:', y);",
    "              unlockScroll();",
    "            }, 300);",
    "          }",
    "          ",
    "          // Manually switch tabs WITHOUT using Bootstrap API",
    "          // This bypasses Bootstrap's internal scroll/focus behavior",
    "          function switchTab(tabElement, targetPane) {",
    "            var tabset = tabElement.closest('.panel-tabset');",
    "            if (!tabset) return false;",
    "            ",
    "            // Find nav and content containers - use :scope to only get direct children",
    "            var navTabs = tabset.querySelector(':scope > .nav-tabs, :scope > ul.nav');",
    "            var tabContent = tabset.querySelector(':scope > .tab-content');",
    "            ",
    "            // Fallback for different DOM structures",
    "            if (!navTabs) {",
    "              navTabs = tabset.querySelector('.nav-tabs, ul.nav');",
    "            }",
    "            if (!tabContent) {",
    "              tabContent = tabset.querySelector('.tab-content');",
    "            }",
    "            ",
    "            if (!navTabs) return false;",
    "            ",
    "            // Deactivate all tabs at this level only",
    "            navTabs.querySelectorAll('[role=\"tab\"], .nav-link').forEach(function(t) {",
    "              t.classList.remove('active');",
    "              t.setAttribute('aria-selected', 'false');",
    "            });",
    "            ",
    "            // Deactivate all panes at this level only",
    "            if (tabContent) {",
    "              // Use :scope to only affect direct children, not nested tab panes",
    "              var directPanes = tabContent.querySelectorAll(':scope > .tab-pane');",
    "              if (directPanes.length === 0) {",
    "                // Fallback if :scope doesn't work",
    "                directPanes = tabContent.querySelectorAll('.tab-pane');",
    "              }",
    "              directPanes.forEach(function(p) {",
    ...
    "            }",
    "            ",
    "            // Dispatch custom event for Highcharts reflow and other listeners",
    "            try {",
    "              tabElement.dispatchEvent(new CustomEvent('shown.bs.tab', {",
    "                bubbles: true,",
    "                detail: { relatedTarget: tabElement }",
    "              }));",
    "            } catch(err) {",
    "              // IE11 fallback - not critical if this fails",
    "            }",
    "            ",
    "            return true;",
    "          }",
    "          ",
    "          // CAPTURE PHASE: Intercept ALL tab clicks before anything else processes them",
    "          document.addEventListener('click', function(e) {",
    "            var tab = e.target.closest('[role=\"tab\"], .nav-link');",
    "            if (!tab) return;",
    "            ",
    "            // Must be inside a panel-tabset",
    ...
    "            ",
    "            // DEBUG: Log that we intercepted the click",
    "            console.log('[dashboardr] Tab click intercepted:', targetId, 'scrollY:', window.scrollY);",
    "            ",
    "            // STEP 1: Lock scroll immediately BEFORE anything else happens",
    "            lockScroll();",
    "            ",
    "            // STEP 2: Prevent ALL default behavior and stop propagation",
    "            e.preventDefault();",
    "            e.stopPropagation();",
    "            e.stopImmediatePropagation();",
    "            ",
    "            // STEP 3: Prevent hash change in URL",
    "            if (history.replaceState) {",
    "              history.replaceState(null, '', window.location.pathname + window.location.search);",
    "            }",
    "            ",
    "            // STEP 4: Switch tab manually (bypass Bootstrap API completely)",
    "            var targetPane = document.querySelector(targetId);",
    "            switchTab(tab, targetPane);",
    "            ",
    "            // STEP 5: Focus without scrolling",
    "            try {",
    "              tab.focus({ preventScroll: true });",
    "            } catch(err) {",
    "              // Fallback: blur to remove focus entirely",
    "              tab.blur();",
    "            }",
    "            ",
    "            // STEP 6: Restore scroll position aggressively",
    "            restoreScrollPosition();",
    "            ",
    "          }, { capture: true, passive: false });",
    "          ",
    "          // Intercept scroll events during tab switching to force position",
    "          document.addEventListener('scroll', function() {",
    "            if (scrollLock.active) {",
    "              console.log('[dashboardr] Scroll blocked! Forcing back to:', scrollLock.y, 'from:', window.scrollY);",
    "              window.scrollTo(scrollLock.x, scrollLock.y);",
    "            }",
    "          }, { passive: false });",
    "          ",
    "          // Intercept shown.bs.tab events at capture phase to prevent Quarto/Bootstrap handlers",
    ...
    "          }, { capture: true });",
    "          ",
    "          // EARLY CAPTURE: Save scroll position on mousedown/pointerdown BEFORE anything else",
    "          // This catches the scroll position before any focus/scroll behavior can change it",
    "          function captureEarlyScroll(e) {",
    "            var tab = e.target.closest('[role=\"tab\"], .nav-link');",
    "            if (!tab) return;",
    "            var tabset = tab.closest('.panel-tabset');",
    "            if (!tabset) return;",
    "            ",
    "            earlyScrollCapture.x = window.scrollX || window.pageXOffset || 0;",
    "            earlyScrollCapture.y = window.scrollY || window.pageYOffset || 0;",
    "            earlyScrollCapture.captured = true;",
    "            console.log('[dashboardr] EARLY capture on', e.type, '- scrollY:', earlyScrollCapture.y);",
    "          }",
    "          ",
    "          // Capture on mousedown (before focus can change scroll)",
    "          document.addEventListener('mousedown', captureEarlyScroll, { capture: true, passive: true });",
    "          document.addEventListener('pointerdown', captureEarlyScroll, { capture: true, passive: true });",
    "          ",
    "        })();",
    "        </script>"
  )
